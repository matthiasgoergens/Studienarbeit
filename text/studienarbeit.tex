...
In a representative the columns will be sorted lexigraphically, for
\(G_{Columns}= S_p\).  The last rows will also be sorted this way.
But we can demand even more.  \todo{Describe two items sorted this
  way, but in the same orbit.}

% Einfach zu machen, da die Gleichheitsprobleme erst in den letzten rows anfangen, und keinen Einfluss auf die Spalten und Zeilen vorher haben.  Oder?
